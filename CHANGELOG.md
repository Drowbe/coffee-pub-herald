...
- **Studio (OBS) menubar button**: a GM-only **Studio** control in Blacksmith's menubar, separate from View Mode, for driving a Coffee Pub Studio automation server directly from Foundry.
  ...
  - **Record Episode / Stop & Upload**: the top of the Studio menu prompts for an Episode Title on record and an Episode Description on stop, sending both to Studio as `heraldTitle`/`heraldDescription` riding Studio's existing `session:StartRecording`/`session:StopRecording` rule sets (not new events of Herald's own) so a chained `uploadToYouTube` rule stage can pick them up. Herald registers its own field keys with Studio (`POST /api/automations/fields`) rather than colliding with Studio's own hand-typed `sessionTitle`/`sessionDescription` fields. Which of the two prompts shows follows the actual recording state, not a guess.
...
- **Transparent `/stream` background** (`streamTransparentBackground`, default on): Foundry hardcodes `body.stream { background: lime }` for chroma key. Herald overrides `html` and `body` to transparent on the capture page so OBS and Studio can composite through it. The MVP widget itself stays opaque. Turning the setting off restores Foundry's lime without a reload.
- **MVP leaderboard stream widget**: Foundry's `/stream` capture page now draws a lifetime MVP ranking so Coffee Pub Studio can crop it into its own OBS source. The root is `#herald-stats` (380 by 320 pixels). Detection matches Blacksmith: `game.view === 'stream'` (toasts) or `body.stream` (loading overlay), and the window is mounted on `init` as well as `ready`. Rank, portrait, name, total, average, and fight count come from Blacksmith's `stats.party.getAggregate()`. Extra rows scroll inside the box; an empty board still leaves the element in the page. The widget does not render on `/game`. See `documentation/architecture/architecture-stream-widgets.md`.

### Changed

- **Stream widgets are Blacksmith tool windows with Auto-Hide chrome**: The MVP board is no longer a hand-built DOM overlay. It subclasses `HeraldStreamWindowBaseV2` (`BlacksmithToolWindowBaseV2` with `toolTitlebar: "auto"`). At rest the title bar is a near-invisible strip; hover/focus expands Full chrome as an overlay so the Studio crop never shifts. Close hides with `visibility` rather than removing `#herald-stats`. Future stream windows should extend that base rather than inventing another overlay.

...
Studio runs its own small HTTPS server, independent of Foundry, on the local network (e.g. `https://10.10.10.x:9500`). Herald is one consumer of its `/api/automations/*` surface, authenticated with a bearer token. Five endpoints:
...
- `POST /api/automations/fields {module, fields: [{key, label}]}` — registers custom field keys Studio does not already know about (see [Custom fields](#custom-fields-record-episode--stop--upload) below). No way to set a Metadata field's *value* remotely, only to register new *keys* that `data` on `/event` can then populate.
...
## Custom fields: Record Episode / Stop & Upload

`POST /api/automations/fields` registers field keys Studio does not already know about. Once registered, those keys become available as sources any `data` sent on `/event` can populate, alongside Studio's own hand-typed Metadata fields — for a downstream rule stage like `uploadToYouTube` to resolve its Title/Description pickers against.

Herald registers `heraldTitle`/`heraldDescription` once per session (`_registerStudioFields()`, called from the first successful `_fetchStudioCapabilities()`, guarded by `_studioFieldsRegistered` and reset when the URL/token settings change). **Deliberately namespaced, not `sessionTitle`/`sessionDescription`**: `resolveDataField` checks Studio's own Metadata fields first and only falls back to event `data[key]` if no Studio field claims that exact key, so reusing Studio's names would always lose to whatever a GM typed into Studio directly.

**Rides Studio's existing `session:StartRecording`/`session:StopRecording` events, not a new pair.** The first version of this feature invented `recording:start`/`recording:stop` and asked Studio to add matching rule sets for them — wrong turn, caught before shipping: Studio already had `session:StartRecording`/`session:StopRecording` rule sets doing the real work (starting OBS, running the scene sequence), already tested end-to-end. Sending a parallel pair of events would either need Studio to duplicate that rule-set logic, or would just not trigger the recording at all. `_startStudioRecordingEpisode()`/`_stopStudioRecordingEpisode()` attach `data` to those existing event names instead — no new Studio-side rule sets needed, only a config change to point `uploadToYouTube`'s Title/Description pickers at `heraldTitle`/`heraldDescription`.

The Studio menu's first item is **Record Episode…** / **Stop & Upload…** (swapped based on the same recording-state poll the indicator uses — never both, never neither). Record prompts for a Title (required; cancelling means recording never starts) and sends `session:StartRecording` with `data: {heraldTitle}`. Stop prompts for a Description (not required — cancelling still stops the recording rather than leaving OBS running indefinitely over a dismissed dialog) and sends `session:StopRecording` with **both** `heraldTitle` and `heraldDescription` in `data`. Both fields travel together on the stop event because that is the specific request whose `data` a chained `uploadToYouTube` stage resolves against — a rule stage only sees the `data` from the one event that triggered its own run, not anything sent earlier on a different event. The title captured at Record time is held in `_studioPendingEpisodeTitle` (a plain static field; it only needs to survive in memory until Stop) and re-sent at Stop for exactly this reason.

Note these two event names are hardcoded (Herald and Studio's existing, already-tested rule sets), unlike the rest of the menu which is built entirely from whatever `ruleSets`/`actions` Studio reports. The two-stage upload chain (stop → wait for the file → upload) is a rule-set/event feature configured on Studio's Automations tab, not something the direct `/action` endpoint supports (it is single-call and synchronous, no chaining), so this could not be built on the generic Controls-flyout Start/Stop Recording actions.

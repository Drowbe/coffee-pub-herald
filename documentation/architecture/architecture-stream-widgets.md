...
Do **not** await `window.BlacksmithAPI.waitForReady()` on `/stream`, even though that looks like the obvious fix and matches what other consumers (Squire) do. Verified live: `window.BlacksmithAPI` exists on `/stream`, but `.isReady` stays `false` for the entire page session — Blacksmith's own `ready`-hook handler starts (far enough to dynamically import `api/blacksmith-api.js`) but never reaches `markReadyForConsumers()` on this page, almost certainly because a later step in that same handler (menubar/UI setup) depends on chrome `/stream` does not render, and hangs. `waitForReady()` only ever resolves, never rejects, so a consumer that awaits it on `/stream` waits forever.

Instead wait for the one concrete precondition this widget needs: Blacksmith's `combatHistory` world setting being registered (`game.settings.settings.get('coffee-pub-blacksmith.combatHistory')`), polled with a bounded timeout. That setting is registered by Blacksmith's `registerSettings()`, called early in the same `ready`-hook handler — before the part that hangs on `/stream` — so it becomes true well before, and independently of, whatever keeps `isReady` from ever flipping. Calling `stats.party.getAggregate()` before it exists throws `"combatHistory is not a registered game setting"` and the widget paints its empty state, with nothing to ever retry it on a page that never fires the actor/combat hooks that would normally invalidate Blacksmith's cache. See `StreamStatsWidget._waitForCombatHistorySetting()` in `scripts/widget-stats.js`.
